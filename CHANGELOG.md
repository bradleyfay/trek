# Changelog
<!-- CI diagnostic: trigger CI to surface macOS failure -->
...
- **Theme-owned surface background** — themes can now optionally define a terminal-wide background colour (`Theme::bg`). This lets palettes such as Norton Commander paint the full surface consistently, including gaps outside widget bounds that would otherwise show the terminal's default background.
...
- **Linux trash: per-device `.Trash-<uid>` directory (XDG spec compliance)** — trashing a file on a filesystem different from `$HOME` now uses `$TOPDIR/.Trash-<uid>/files/` instead of crossing a device boundary with a slow copy+delete. The filesystem mount point is found by walking up the directory tree comparing `st_dev`. The correct trash directory is selected by comparing `st_dev` of the file against `st_dev` of `$HOME`; same-device files continue to use `$XDG_DATA_HOME/Trash/files/`. `.trashinfo` sidecars are written correctly for both layouts. macOS behavior is unchanged (#146).
- **Decomposed `App` god-struct into focused sub-structs** — the 90+ field flat `App` struct is reorganised into three focused sub-structs: `NavigationState` (`nav`) — directory listing, cursor, history, sort, filter, search, selection, and frecency log; `PreviewState` (`preview`) — preview content, scroll, mode flags, and focus state; `OverlayState` (`overlay`) — all modal overlay UI state (palette, bookmarks, find, symlink, dup, cmux-surface-picker, and more). `App::new` delegates to sub-struct constructors. Infrastructure fields (terminal size, layout dividers, watcher, git status, clipboard, task manager) remain directly on `App` (#132).
...
- **Thread-safe session and bookmark tests** — `session.rs`, `bookmarks.rs`, and `app/tests.rs` previously used `std::env::set_var` / `remove_var` to redirect `XDG_DATA_HOME` during tests. These calls are unsound in parallel test runs (undefined behavior per Rust's docs) and will require `unsafe` in a future edition. Introduced `load_from(path)` / `save_to(path, ...)` helpers in both `session` and `bookmarks` modules; all tests now pass explicit per-test temp paths and make no env mutations.
